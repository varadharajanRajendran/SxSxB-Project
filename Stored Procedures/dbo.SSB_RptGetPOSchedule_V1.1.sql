...
CREATE  PROCEDURE [dbo].[SSB_RptGetPOSchedule_V1.1]		
		...
		BEGIN
			...
			BEGIN
				...
				WHERE SKU=@CurrentSKU 
						AND TruckID=@CurrentTruck
						AND ShipmentDate=@SelTruckDate
						AND ShipmentTime=@SelTruckTime
				
				SELECT @CurrentTruck,@SelTruckTime,@SelTruckDate
				SELECT * FROM @tblPOProperty
