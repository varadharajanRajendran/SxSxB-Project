...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[SSB_PPA_GetCTQData]
...
INSERT INTO @tblCTQDAta	(CTQDescription,DataType,Actual,LTL	,LPCL,[Target],UPCL,UTL, VLD,Mandatory	)  
	SELECT DISTINCT REPLACE(PS.[name],'QUALITY_','')
		  ...
		  ,ISNULL(NULLIF(CONVERT(float,PS.[default_val] )- CONVERT(float, PM.[LOW_MIN] ),' '), '0') 
		  ,ISNULL(NULLIF(CONVERT(float, PS.[default_val]) - CONVERT(float, PV.ParamValues_MinVal ),' '), '0') 
		  ,ISNULL(NULLIF(PS.[default_val],' '), '0')
		  ,ISNULL(NULLIF(CONVERT(float, PV.ParamValues_MaxVal)+CONVERT(float, PS.[default_val]),' '), '0') 
		  ,ISNULL(NULLIF(CONVERT(float, PM.[HIGH_MAX])+CONVERT(float, PS.[default_val]),' '), '0') 
		  ,PS.[VLD]
		  ,PM.Param_Mandatory
	  FROM [SitMesDB].[dbo].[POM_PROCESS_SEGMENT_PARAMETER] PS
	   ...
	   INNER JOIN sitmesdb.dbo.pdefm_ps_ParamValues PV ON PV.ParamValues_num=PM.Param_ValuesLink
