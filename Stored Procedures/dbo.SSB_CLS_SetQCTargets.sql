...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[SSB_CLS_SetQCTargets]
...
AS
 ...
 DECLARE @PPR_name				nvarchar(255)	,
		 ...
		 @QUALITY_BtmLength		nvarchar(255)	,
		 @QUALITY_TopLength		nvarchar(255)	
...
SELECT @PPR_name=[ppr_name],
  ...
  /*
SELECT @QUALITY_BtmLength=[Length],
	   @QUALITY_TopLength=[Length]
...
UPDATE [SitMesDB].[dbo].[POM_PROCESS_SEGMENT_PARAMETER]
	...
	WHERE  pom_entry_pk=@EntryID
		AND Name='QUALITY_BtmLength'

UPDATE [SitMesDB].[dbo].[POM_PROCESS_SEGMENT_PARAMETER]
	SET Default_Val='50'
	WHERE  pom_entry_pk=@EntryID
		AND Name='QUALITY_TopLength'
