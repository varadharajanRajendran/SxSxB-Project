...
CREATE  PROCEDURE [dbo].[PLC_UtilityGenerateDataSet]
		@ProdLineName		NVARCHAR(20),
		@StatusID			NVARCHAR(255),
		@TransactionType	NVARCHAR(20),
		@ResendData			NVARCHAR(20)
AS

/*
Purpose   : MES to Line Control Interface Data Preparation
Author    : Varadha R (Varadharajan.Rajendran@b-wi.com)
Revision  : Initial (July.22.2015)
            11-11-2015 - Script modified to handle all PCL,HYL,CML and SPL Line
...
DELETE FROM SSB.dbo.Temp_PoOrder
INSERT INTO SSB.dbo.Temp_PoOrder (orderID,Seq)
...
BEGIN
	IF @TransactionType='BC'
		BEGIN
			DELETE [SSB].[dbo].[Temp_PoOrder]
			FROM [SSB].[dbo].[Temp_PoOrder] Po
				INNER JOIN SitmesDB.dbo.POMV_ORDR_PRP_VAL Prop ON Prop.pom_order_id=Po.OrderID
			   Where pom_custom_fld_name='SetBCDownload'
			EXEC SSB.dbo.LS_FTAssignDYNLocation @ProdLine=@ProdLineName
			EXEC SSB.dbo.LS_FEAssignLocation @ProdLine=@ProdLineName
		END
	ELSE IF @TransactionType='DYN'
		BEGIN
			...
			FROM SSB.dbo.Temp_PoOrder Po
				INNER JOIN SitmesDB.dbo.POMV_ORDR_PRP_VAL Prop ON Prop.pom_order_id=Po.OrderID
			   Where pom_custom_fld_name='SetDYNDownload'
		END
END

SELECT @Counter=COUNT(RowID) FROM SSB.dbo.Temp_PoOrder 
IF @Counter=0 
BEGIN
...
BEGIN TRY	
	BEGIN /* Create Table */	
		...
		END
		EXEC ('DROP TABLE [SSB].[dbo].[PLC_BC_'+ @ProdLineName + ']') 
		SET @SQLStringCreateTable = 'CREATE TABLE [SSB].[dbo].[PLC_BC_'+ @ProdLineName + '] ([RowID] [int] IDENTITY(1,1) NOT NULL,'
	...
	BEGIN /* Get Order List, SKU and Description   */	
		SET @SQLStringCreateTable = 'INSERT INTO [SSB].[dbo].[PLC_BC_'+ @ProdLineName + '](JobID,SKUNo)
			...
			FROM [SitMesDB].[dbo].[POM_ENTRY] Pe
				...
				INNER JOIN [SSB].[dbo].Temp_PoOrder O ON O.OrderID=Po.Pom_order_id
		...
		EXEC (@SQLStringCreateTable)
		SET @SQLStringCreateTable = 'UPDATE [SSB].[dbo].[PLC_BC_'+ @ProdLineName + '] 
										SET Product= MMD.Descript
										FROM [SSB].[dbo].[PLC_BC_'+ @ProdLineName + '] Po
	...
	BEGIN /* Get Order Properties   */
		...
		BEGIN
			...
			SET @SQLStringCreateTable = 'UPDATE [SSB].[dbo].[PLC_BC_'+ @ProdLineName + ']
										 ...
										 FROM  [SSB].[dbo].[PLC_BC_'+ @ProdLineName + '] AS o 
		...
		BEGIN
			...
			SET @SQLStringCreateTable = 'UPDATE [SSB].[dbo].[PLC_BC_'+ @ProdLineName + ']
										 ...
										 FROM  [SitMesDB].dbo.MMvBomAltPrpVals AS BAPV WITH (NOLOCK) 
											...
											INNER JOIN  [SSB].[dbo].[PLC_BC_'+ @ProdLineName + '] Po ON Po.SKUNo=D.[DefID]
	...
	BEGIN /* Get MU Properties and Locations */
		...
		BEGIN  /* Get MU Properties */ 
			...
			FROM @DistinctFields
			WHERE RowID=@StartRow
			EXEC [SSB].[dbo].[PLC_UtilityGetEntryProperties] @ProdLine=@ProdLineName,@Entry='MU1' ,@SelField=@Selfield, @TransactionType='BC'
			SELECT @startRow=@startRow+1
		END
	    BEGIN  /* Part Description and Pick Location */	
			...
			BEGIN
				SET @SQLStringCreateTable = 'UPDATE [SSB].[dbo].[PLC_BC_'+ @ProdLineName + ']
												...
												FROM  [SSB].[dbo].[PLC_BC_'+ @ProdLineName + '] AS LCD 
		...
		BEGIN
			...
			WHILE @startRow<=@EndRow
				BEGIN  /* Get Properties */ 
					...
					FROM @DistinctFields
					WHERE RowID=@StartRow
					EXEC [SSB].[dbo].[PLC_UtilityGetEntryProperties] @ProdLine=@ProdLineName,@Entry='FEC1' ,@SelField=@Selfield, @TransactionType='BC'
					SELECT @startRow=@startRow+1
				END	
				BEGIN /* Get BF_PartNo and Location*/
			EXEC (' UPDATE [SSB].[dbo].[PLC_BC_'+ @ProdLineName + ']
											SET  [BFPNo]=ml.def_id 
											FROM   [SSB].[dbo].[PLC_BC_'+ @ProdLineName + '] AS o 
	...
	BEGIN /* Get CUI Properties and Locations */
		...
		BEGIN  /* Get Properties */ 
			...
			FROM @DistinctFields
			WHERE RowID=@StartRow
			EXEC [SSB].[dbo].[PLC_UtilityGetEntryProperties] @ProdLine=@ProdLineName,@Entry='CUI1' ,@SelField=@Selfield, @TransactionType='BC'
			SELECT @startRow=@startRow+1
		END	
	END	
	BEGIN	
		EXEC (' UPDATE [SSB].[dbo].[PLC_BC_'+ @ProdLineName + ']
					...
					FROM  [SitMesDB].[dbo].POM_ENTRY AS Pe 
						INNER JOIN [SSB].[dbo].[PLC_BC_'+ @ProdLineName + '] AS LC ON LC.JobID=Pe.Pom_entry_id
						INNER JOIN [SitMesDB].[dbo].POM_CUSTOM_FIELD_RT AS ocf_rt ON Pe.pom_entry_pk = ocf_rt.pom_entry_pk 
						INNER JOIN [SitMesDB].dbo.POM_CF_VALUE_RT AS ocf_val ON ocf_rt.pom_custom_field_rt_pk = ocf_val.pom_custom_field_rt_pk
