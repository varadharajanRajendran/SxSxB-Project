...
CREATE  PROCEDURE [dbo].[SSB_RptGetLCPLC_new]		
 AS
 DELETE FROM [SSB].[dbo].MES2LC
 EXEC [dbo].[SSB_Update_tblMES2LC_new]

SELECT  ISNULL(LC.FECType,'')						as 'FEC TYPE'    	,
		LC.[SKUNo]									as 'SKU'			,
		...
		''											as 'PL'				,
		''											as 'BL'				,
		ISNULL(LC.PQ	,0)								as 'PanelQuilt'		,
		LC.[DueDate]									as 'DueDate'		,
        LC.[DueTime]									as 'DueTime'		,
		LC.[EstTime]									as 'EstimatedStartTime',
...
FROM  [SitMesDB].dbo.MMvBomAltPrpVals AS BAPV WITH (NOLOCK) 
	  ...
	  INNER JOIN  [SSB].[dbo].[MES2LC] LC ON LC.SKUNo=D.[DefID]
