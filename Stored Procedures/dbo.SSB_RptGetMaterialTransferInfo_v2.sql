...
CREATE  PROCEDURE [dbo].[SSB_RptGetMaterialTransferInfo_v2]	
...
INSERT INTO @tblOrders(OrderID)					
    ...
    WHERE Pos.id= 'Production'
            ...
            AND ocf2_rt.pom_custom_fld_name='ShipmentTime') AS o
            INNER JOIN [SitMesDB].[dbo].POM_ORDER AS  Po ON Po.Pom_order_id=o.pom_order_id
            INNER JOIN [SitMesDB].[dbo].POM_ENTRY AS Pe ON Pe.Pom_order_pk = Po.Pom_order_pk
            INNER JOIN [SitMesDB].[dbo].[POM_ORDER_STATUS] PoS ON PoS.[pom_order_status_pk]=Po.[pom_order_status_pk]
    GROUP BY Po.Pom_order_id
    having MAX( DATEADD(minute,-Pe.[estimated_end_time_bias],Pe.[estimated_end_time])) is not null
    order by MAX( DATEADD(minute,-Pe.[estimated_end_time_bias],Pe.[estimated_end_time]))
UPDATE @tblOrders
	SET GroupID =CASE
