...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[SSB_PPA_GetCTQData_V2]
...
INSERT INTO @tblCTQDAta	(CTQDescription,DataType,Actual,LTL	,LPCL,[Target],UPCL,UTL, VLD,Mandatory	)  
	SELECT REPLACE(PS.[name],'QUALITY_','')
		  ,PS.[type]
		  ,ISNULL(NULLIF(PS.[value],' '), '0')
		  ,ISNULL(NULLIF(CONVERT(float, PS.[LOW_MIN])+CONVERT(float, PeRT.[VAL]),' '), '0') 
		  ,ISNULL(NULLIF(CONVERT(float, PS.[min_val])+CONVERT(float, PeRT.[VAL]),' '), '0') 
		  ,ISNULL(NULLIF(PS.[default_val],' '), '0')
		  ,ISNULL(NULLIF(CONVERT(float, PS.[max_val])+CONVERT(float, PeRT.[VAL]),' '), '0') 
		  ,ISNULL(NULLIF(CONVERT(float, PS.[HIGH_MAX])+CONVERT(float, PeRT.[VAL]),' '), '0') 
		  ,PS.[VLD]
		  ,PM.Param_Mandatory
	  FROM [SitMesDB].[dbo].[POM_PROCESS_SEGMENT_PARAMETER] PS
	   ...
	   INNER JOIN [SitMesDB].[dbo].[POM_CUSTOM_FIELD_RT] PeRT on PeRT.pom_entry_pk= PE.pom_entry_pk
	  Where PE.pom_entry_id like  @EntryID +'%'
	   ...
	   AND PeRT.[pom_custom_fld_name]=REPLACE(PS.[name],'QUALITY_','PROD_')
	  Order by PS.seq ASC
