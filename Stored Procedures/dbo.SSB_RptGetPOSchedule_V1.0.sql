...
CREATE  PROCEDURE [dbo].[SSB_RptGetPOSchedule_V1.0]		
...
/*
DECLARE @SelOrders				nvarchar(255)	,
	    @StartDate				date			,
		@EndDate				date

SELECT @SelOrders	= 'Manual Line'	,
...
SELECT @SelOrderStatus =CASE @SelOrders
							WHEN 'New Orders' THEN 'Downloaded'
							WHEN 'Manual Line'	 THEN 'Distributed_OHM'
							WHEN 'Automated Line' THEN 'Scheduled'
						END

DELETE FROM  [SSB].[dbo].[POSchedule]
BEGIN  /* Extract Data */
	...
	INSERT INTO @tblPOProperty (OrderID,ShipmentDate	) /* ShipmentDate */
		SELECT  o.OrderNo,CONVERT(nvarchar(50),ocf_val.pom_cf_value)
			FROM  @tblOrder  AS o 
			...
			WHERE ocf_rt.pom_custom_fld_name='ShipmentDate'
				AND CONVERT(date,ocf_val.pom_cf_value)>=@StartDate
				AND CONVERT(date,ocf_val.pom_cf_value)<=@EndDate
	...
	INSERT INTO @tblTruck (TruckID,ShipmentDate,ShipmentTime)
		...
		FROM  @tblPOProperty
	INSERT INTO @tblSKU (SKU	,[Description]	,UnitType,[Size],[BedType],[CoreType],Panel	,BorderDec	,MU	,NoofLayers,NoofSides,L1PartDesc,L2PartDesc,L3PartDesc	,L4PartDesc	,L5PartDesc	,L6PartDesc)
		SELECT DISTINCT SKU	,[Description]	,UnitType,[Size],[BedType],[CoreType],Panel	,BorderDec	,MU	,NoofLayers,NoofSides,L1PartDesc,L2PartDesc,L3PartDesc	,L4PartDesc	,L5PartDesc	,L6PartDesc		
		FROM  @tblPOProperty
END
...
BEGIN /* Truck Data Manuplation */
	...
	BEGIN
		...
		BEGIN
			...
			WHERE RowID=@intStartTruck
			SELECT @OrderCount	= COUNT(RowID) FROM @tblPOProperty
			WHERE SKU=@CurrentSKU AND TruckID=@CurrentTruck
			SET @SQLStringInsertDate= 'UPDATE [SSB].[dbo].[POSchedule] SET [' + @CurrentTruck + ']=' + char(39) + CONVERT (nvarchar(20),@OrderCount) + char(39) + '   WHERE SKU= ' + char(39) + @CurrentSKU + char(39) 
			EXEC (@SQLStringInsertDate)
			SELECT @intStartTruck=@intStartTruck +1
		END
