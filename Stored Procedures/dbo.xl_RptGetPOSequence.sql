...
CREATE PROCEDURE [dbo].[xl_RptGetPOSequence]
...
BEGIN	/* Preliminary Data Preparation*/
	INSERT INTO @tblOrder (OrderID,SKU	,TruckID,ShipmentDate,ShipmentTime,Wave	,UnitType,Seq)
		SELECT	Po.Pom_order_id								,
				Pe.[matl_def_id]							,
				CONVERT(nvarchar(20),ocf3_val.pom_cf_value) ,
				CONVERT(nvarchar(20),ocf1_val.pom_cf_value) ,
				CONVERT(nvarchar(20),ocf2_val.pom_cf_value) ,	
				CONVERT(nvarchar(20),ocf4_val.pom_cf_value) ,
				CONVERT(nvarchar(20),ocf5_val.pom_cf_value) ,
				CONVERT(int,ocf7_val.pom_cf_value)
		FROM [SitMesDB].[dbo].POM_ORDER AS  Po 
			...
			LEFT OUTER JOIN [SitMesDB].[dbo].POM_CUSTOM_FIELD_RT AS ocf6_rt ON Pe.pom_entry_pk = ocf6_rt.pom_entry_pk 
			LEFT OUTER JOIN [SitMesDB].[dbo].POM_CUSTOM_FIELD_RT AS ocf7_rt ON Pe.pom_entry_pk = ocf7_rt.pom_entry_pk 
			...
			LEFT OUTER JOIN [SitMesDB].dbo.POM_CF_VALUE_RT AS ocf7_val ON ocf7_rt.pom_custom_field_rt_pk = ocf7_val.pom_custom_field_rt_pk
		WHERE ocf1_val.pom_cf_value =@ShipmentDate
			...
			AND ocf6_rt.pom_custom_fld_name='ActualLine'
			AND ocf6_val.pom_cf_value=@ProdLine
			AND ocf7_rt.pom_custom_fld_name='PreactorSequence'
		ORDER BY ocf7_val.pom_cf_value ASC
	UPDATE @tblOrder
	...
	IF @DataCount>0
	BEGIN
		INSERT INTO @tblOrderSeq (OrderID,SKU,TruckID,ShipmentDate,ShipmentTime,Wave,UnitType,Seq,CoilDesc)
			SELECT o.OrderID		,
				   o.SKU			,
				   o.TruckID		,
				   o.ShipmentDate	,
...
BEGIN   /*  Get SKU Details */		
	SELECT @DataCount = COUNT(*) FROM @tblOrderSeq 
	IF @DataCount>0
	BEGIN
		UPDATE @tblOrderSeq 
...
BEGIN   /*  table Data Presentation  */
	...
	END
	SELECT @DataCount = COUNT(*) FROM @tblOrdersbySKU
	IF @DataCount>0
	BEGIN
