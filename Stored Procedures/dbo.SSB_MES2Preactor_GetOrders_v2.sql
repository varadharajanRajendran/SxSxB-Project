...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[SSB_MES2Preactor_GetOrders_v2]	
AS
	...
	INSERT INTO Temp_MES2Preactor (PID,OrderID,ProcessType,PlantNo,EntryID,CustomerNo,FGPart,[Quantity])
		SELECT  CASE Pe.[pom_entry_id]
					...
					WHEN (Po.[pom_order_id] + '.Gussett1') THEN '4'
					WHEN (Po.[pom_order_id] + '.Gussett2') THEN '4'
					WHEN (Po.[pom_order_id] + '.THC1') THEN '5'
					WHEN (Po.[pom_order_id] + '.THC2') THEN '5'
					WHEN (Po.[pom_order_id] + '.BHC1') THEN '6'
					WHEN (Po.[pom_order_id] + '.BHC2') THEN '6'
					WHEN (Po.[pom_order_id] + '.CU1') THEN '7'
					WHEN (Po.[pom_order_id] + '.SBCoil1') THEN '8'
					WHEN (Po.[pom_order_id] + '.SBCoil2') THEN '8'
					WHEN (Po.[pom_order_id] + '.SBCoil3') THEN '8'
					WHEN (Po.[pom_order_id] + '.SBCoil4') THEN '8'
				END
			  ,Po.[pom_order_id]
			  ,CASE Pe.[pom_entry_id]
					...
					WHEN (Po.[pom_order_id] + '.Gussett1') THEN 'Gussett'
					WHEN (Po.[pom_order_id] + '.Gussett2') THEN 'Gussett'
		  ...
		  WHERE PeS.id IN('Scheduled','Initial','Download','Scheduled')
			AND PoS.id in('Download','Initial','To Be Scheduled')
			AND (Pe.pom_entry_id=Po.pom_order_id
					OR  Pe.pom_entry_id  like'%.Gussett1'
					OR  Pe.pom_entry_id  like'%.Gussett2'
	...
	*/
	...
	UPDATE Temp_MES2Preactor
		SET FGPart= Pe.matl_def_id 
		FROM [SitMesDB].[dbo].[POM_ENTRY] Pe
			INNER JOIN [SitMesDB].[dbo].[POM_ORDER] Po ON Pe.Pom_Entry_id=Po.Pom_order_id
			INNER JOIN Temp_MES2Preactor TempM2P	ON TempM2P.OrderID= Po.Pom_order_id
